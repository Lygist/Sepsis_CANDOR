...
def evaluate_phi_loglikelihood(env, model, num_episodes=50, discount=0.99, deterministic=False,  device='cpu', baseline=0, v_net=None):
    ...
    while count < num_episodes:
        ...
        if v_net is not None:
            # For IQL, use V-net as baseline
            baselines = v_net(torch.cat(state_list, dim=0).float()).detach().squeeze()
        else:
...
def compute_policy_hessian_inverse_vector_product(x_raw, env_parameter, env, model, num_episodes=50, discount=0.99, baseline=0, verbose=0, backprop_method=1, v_net=None):
    start_time = time.time()
    if backprop_method in [0,1,2,3]:
        phi_list, total_log_likelihood_list, total_transition_log_likelihood_list, _ = evaluate_phi_loglikelihood(env, model, num_episodes=num_episodes, discount=discount, baseline=baseline, v_net=v_net)
        c = -1
...
# ===================================== implementation of sequential CANDOR==============================
def seq_CANDOR(env, policy_kwargs, trajectories, discount=0.95, device='cpu', variant='DM+-IS'):
    class SeqCANDORFn(torch.autograd.Function):
        @staticmethod
        def forward(ctx, model_parameters):
            with torch.enable_grad():
                model = DQN("MlpPolicy", env, policy_kwargs=policy_kwargs, verbose=0, gamma=discount, buffer_size=0, device=device)
                ...
                # Selected Q = \hat{Q}^+(s_t, a_t)
                selected_q = torch.gather(q_values, dim=1, index=actions).view(N, T)

                # Recursion for V per trajectory, backward
                ...
                for t in range(T-1, -1, -1):
                    ...
                    rho_t = rho[:, t]
                    q_error = rewards[:, t] + discount * V[:, t+1] - selected_q[:, t]
                    correction = rho_t * q_error
                ...
                value_gradient = torch.cat([param.grad.flatten() for param in model.policy.q_net.parameters()]).detach()
                value_gradient = torch.cat([value_gradient, torch.zeros_like(value_gradient)])

                value_gradient[value_gradient.isnan()] = 0
                model.policy.optimizer.zero_grad()

            ctx.save_for_backward(value_gradient)
            for param in model.policy.parameters():
                del param.grad
        ...
        def backward(ctx, dl_dvalue):
            value_gradient, = ctx.saved_tensors
            return dl_dvalue * value_gradient

    return SeqCANDORFn.apply
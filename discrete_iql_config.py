...
from d3rlpy.algos.qlearning.torch.iql_impl import (
    DiscreteIQLImpl,
    DiscreteIQLModules,
    IQLImpl,
    IQLModules,
...
class IQL(QLearningAlgoBase[IQLImpl, IQLConfig]):
    def inner_create_impl(
        ...
        actor_optim = self._config.actor_optim_factory.create(
            policy.named_modules(), lr=self._config.actor_learning_rate,compiled=self.compiled,
        )
...
class DiscreteIQLConfig(LearnableConfig):
    ...
    def create(self, device: DeviceArg = False, enable_ddp: bool = False) -> "DiscreteIQL":
        return DiscreteIQL(self, device, enable_ddp=False)
...
class DiscreteIQL(QLearningAlgoBase[DiscreteIQLImpl, DiscreteIQLConfig]):
    def inner_create_impl(
        ...
        v_func_params = list(value_func.named_modules())
        critic_optim = self._config.critic_optim_factory.create(
            q_func_params + v_func_params, lr=self._config.critic_learning_rate,compiled=self.compiled
        )
        actor_optim = self._config.actor_optim_factory.create(
            policy.named_modules(), lr=self._config.actor_learning_rate, compiled=self.compiled,
        )
